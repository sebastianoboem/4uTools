...
cp "$TARGET/aarch64-apple-darwin/release/bundle/dmg/4uTools_${VERSION}_aarch64.dmg" \
  "$OUT/4uTools_${VERSION}_arm64.dmg"

echo "==> Build macOS Intel (x64)"
npm run tauri build -- --target x86_64-apple-darwin
cp "$TARGET/x86_64-apple-darwin/release/bundle/dmg/4uTools_${VERSION}_x64.dmg" \
  "$OUT/4uTools_${VERSION}_x64.dmg"

echo "==> Build Windows (x64)"
npm run tauri build -- --runner cargo-xwin --target x86_64-pc-windows-msvc
cp "$TARGET/x86_64-pc-windows-msvc/release/bundle/nsis/4uTools_${VERSION}_x64-setup.exe" \
